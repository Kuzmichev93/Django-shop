...
def send_messange(request):
    if request.method == 'GET':
        ...
        if request.POST['header'] and request.POST['email'] and request.POST['content']:
            ...
            return render(request,'banksite/send_messange.html')
    ...
    if request.method=="GET":
        ...
        try:
            if len(request.GET["search"])>0:
                 ...
                 men = men_shoes.objects.filter(title__icontains=request.GET["search"])
                 wom = women_shoes.objects.filter(title__icontains=request.GET["search"])
                 if men and wom:
